...
**交易状态机**：
```
待处理(0) ──卖家接受──> 已接受(1) ──买家输码确认──> 已完成(3)
    │                       │
    └──卖家拒绝──> 已拒绝(2)  └──其他请求自动拒绝──> 已拒绝(2)
```
...
**实现文件**：[admin.py](file:///c:/Users/yapeng/OneDrive/Desktop/EcoMarket/app/api/v1/admin.py)

**实现要点**：
- `get_admin_user` 依赖：校验 `user_id == 1` 视为管理员
- 用户管理：获取全部用户列表
- 商品审核：管理员可修改商品状态（下架等）
- 平台统计：用户数、商品数、待处理订单数
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/admin/users` | 获取全部用户列表 |
| PUT | `/api/v1/admin/items/{item_id}/audit` | 商品审核 |
...
本项目采用模块化分工，各成员根据专长承担不同模块的设计与实现。具体分工如下：

### 6.1 分工表

| 成员 | 主要职责 | 涉及模块/文件 |
|------|---------|---------------|
| 成员 A | 后端架构设计、数据库设计 | `app/core/` (config, database, redis, security)、`app/models/` (7 张表模型)、Alembic 迁移 |
| 成员 B | 商品管理、搜索优化 | `app/api/v1/items.py`、`app/services/item_service.py`、`app/services/search_compat.py`、索引基准脚本 |
| 成员 C | 交易系统、事务与行锁 | `app/services/request_service.py`、`app/api/v1/requests.py`、交易码确认机制、压测脚本 |
| 成员 D | 前端开发、数据可视化 | `frontend/src/` (10 个页面、4 个组件、11 个 API 模块)、ECharts 图表 |
| 成员 E | AI 模块、消息系统 | `app/services/ai_service.py`、`app/api/v1/ai.py`、`app/api/v1/messages.py`、基于商品的对话设计 |
| 成员 F | 测试与文档 | `tests/` (36 个单元测试)、项目报告、README、.gitignore |

### 6.2 协作流程

1. **需求阶段**：共同讨论确定功能模块划分、数据库表结构、API 接口规范
2. **设计阶段**：成员 A 主导数据库设计，成员 D 主导前端原型设计，共同评审
3. **开发阶段**：各成员按模块独立开发，通过 Git 分支协作，每日同步
4. **联调阶段**：前后端接口对接，修复联调问题
5. **测试阶段**：成员 F 编写单元测试，全员参与功能测试与性能验证
6. **文档阶段**：各成员补充所负责模块的文档内容，成员 F 统一整理报告

### 6.3 版本管理

- 使用 Git 进行版本控制
- 主分支 `main`，开发分支 `develop`，功能分支 `feature/模块名`
- 通过 Pull Request 合并代码，需至少一人 Code Review
- 定期清理无用的分支和临时文件
...
**困难描述**：
按照原计划使用 `docker pull mysql:8.0` 拉取镜像时，由于网络原因 Docker Hub 无法访问，导致 MySQL 容器无法启动。

**解决方案**：
配置国内镜像加速源（阿里云、腾讯云），或改用本地安装 MySQL 9.7。最终选择本地安装 MySQL，openGauss 仍使用 Docker。

### 7.8 PowerShell 多行 SQL 命令转义问题

**困难描述**：
在 PowerShell 中执行多行 SQL 命令（如数据清理）时，引号和换行符解析错误，导致 SQL 语法错误。

**解决方案**：
改用 Python 脚本文件代替内联命令，通过 SQLAlchemy 执行数据库操作，避免 shell 转义问题。

### 7.9 前端图片加载失败（ORB 拦截）

**困难描述**：
商品图片从 unsplash.com 加载时，浏览器控制台报 `ERR_BLOCKED_BY_ORB`（ORB: Cross-Origin Resource Blocking），图片无法显示。

**根因分析**：
unsplash.com 的图片响应头不符合浏览器同源策略要求，被 ORB 拦截。

**解决方案**：
- 将图片源更换为 picsum.photos
- 在 ItemCard 组件添加 `@error` 容错处理，图片加载失败时显示「暂无图片」占位
...
| 前端相关 | 2 | 图片 ORB 拦截、对话弹窗设计 |
| 环境与工具 | 2 | Docker Hub 不可用、PowerShell 转义 |
